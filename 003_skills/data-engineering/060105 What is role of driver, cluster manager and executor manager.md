...
> [!Summary] Summary
> - The driver
> 	-	It is a process that run the main application
> 	-	It performs analysis converts high level code into a logical plan then to a physical plan called DAG
> 	-	It breaks DAG into stage and tasks then schedules these tasks to run on executor
> 	-	co-ordinate with all the executors for task status and re-schedule if the task fails
> 	-	It collects the data when we execute an action.
> - Cluster manager
> 	-	cluster manage is external to the spark it manages the physical resource of the cluster
> 	-	Spark supports YARN, Kubernetes and Mesos
> 	-	it is responsible for resource allocation, isolation
> 	-	Handoff
> - The Executor
> 	-	The executor is a distributed process that lives on a worker node
> 	-	It executes the task, it provides storage to cache and persist
> 	-	Reports heartbeat and also send the result
> 
> 
>**How it works?**
> -	When a code is submitted a session is created by the driver. The driver creates the DAG and based on task it creates stages and task
> -	Once task is created the driver ask the cluster manager for resources
> -	Once resource is found - process is start on node (executor)
> -	driver sends task to executors; executor process the data and sends back results to driver.
